...
<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=91bad6a8a2d77f97dfddb38d7a1448f6&sql=SELECT+a.%2A%2Cb.truename+FROM+v9_trade+a%2Cv9_member_detail+b+WHERE+a.username+%3D+b.userid+and+a.type+%3D+2+and+a.status+%21%3D3+and+a.status+%21%3D0+and+a.username+%3D+%27%24userid%27+order+by+a.price+desc%2Ca.id+desc++&num=10&return=info\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT a.*,b.truename FROM v9_trade a,v9_member_detail b WHERE a.username = b.userid and a.type = 2 and a.status !=3 and a.status !=0 and a.username = '$userid' order by a.price desc,a.id desc   LIMIT 10");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$info = $a;unset($a);?> 
