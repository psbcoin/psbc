 ...
 * @lastmodify			2010-6-1
 */
function passwords($str='',$re=''){
	 return md5(md5(trim($str)).$re);
}
...
function rand_code() {
    ...
    for(
        $a = md5( $rand, true ),
        $s = '23456789ABCDEFGHJKLMNPQRSTUV',
        $d = '',
        $f = 0;
        $f < 8;
        $g = ord( $a[ $f ] ),
...
//是否为下级
function is_tuan($userid,$uid){
	$data = run_sql("SELECT userid FROM  `v9_member` where tjuser = '$userid' ");
	if($data){
		$ch++;
		foreach($data as $k => $v){
			if($uid==$v['userid']){
				...
				$ck = is_tuan($v['userid'],$uid);
				if($ck) return 1;
			}
		}
	} 
}
function is_tuan1($userid,$uid,$arr = array()){
	$data = run_sql("SELECT userid FROM  `v9_member` where tjuser = '$userid' ");
	if($data){
		foreach($data as $k => $v){
			if($uid == $v['userid']){
			echo $v['userid']."=>".$uid."<br>"; 
				return 1;
			}else{
				echo $v['userid']."<=>".$uid."<br>"; 
				$ck = is_tuan1($v['userid'],$uid);
				if($ck) return 1;
			}
		}
	}
...
//团队余额 10级
function tuan_pris($uid,$num = 0,$ch=1){
	//	echo $ch."=>".$uid."=>".$num."<br>";
	if($ch >5) return $num;
	$data = run_sql("SELECT userid,amount FROM  `v9_member` where tjuser = '$uid' ");
	if($data){
		$ch++;
		foreach($data as $k => $v){
			$num = $num + $v['amount']-100;
			//echo $v['userid']."=>".$v['amount']."=>".$num."<br>";
			$num = tuan_pris($v['userid'],$num,$ch);
		}
		return $num;
	}else{
		return $num;
...
//团队数量 10级
function tuan_nums($uid,$num = 0,$ch =1){
	//	echo $ch."=>".$uid."=>".$num."<br>";
	if($ch >5) return $num;
	$data = run_sql("SELECT userid FROM  `v9_member` where tjuser = '$uid' ");
	if($data){
		$ch++;
		foreach($data as $k => $v){
			$num++;
			$num = tuan_nums($v['userid'],$num,$ch);
		}
		return $num;
	}else{
		return $num;
...
function run_sql($sql){
	$thiss = pc_base::load_model('content_model');
	$ret=$thiss->query($sql);
	$data = $thiss->fetch_array($ret);
	return $data;
}
function run_sql_one($sql){
	$thiss = pc_base::load_model('content_model');
	$ret=$thiss->query($sql." limit 1");
	$data = $thiss->fetch_array($ret);
	return $data[0];
}
function save_sms($fone,$code,$uid=0){
	$time = time();
	run_sql("INSERT INTO `v9_a_sms` (`sid`, `uid`, `fone`, `code`, `addtime`, `stat`) VALUES (NULL, '$uid', '$fone', '$code', '$time', '0')");
	return true;
}
function ck_sms($fone,$code=''){
	$time = time()-600;
	$isck = run_sql_one("SELECT * FROM  `v9_a_sms` where  fone='$fone' and code='$code' and stat = 0 and addtime > $time order by sid desc");
	if($isck){
		run_sql("UPDATE  `v9_a_sms` SET  `stat` =  '1' WHERE `sid` ='".$isck['sid']."';");
		return true;
	}else{
		return false;
	}
}
function send_sms($fone,$type=1){
	$time = time();
	$time1 = time()-600;
	$code = rand(1000,9999);
	$isck = run_sql_one("SELECT * FROM  `v9_a_sms` where  fone='$fone' and stat = 0 and addtime > $time1 order by sid desc");
	if($isck){
		$code = $isck['code'];
	}else{
		run_sql("INSERT INTO  `v9_a_sms` (`sid`, `uid`, `fone`, `code`, `addtime`, `stat`) VALUES (NULL, '0', '$fone', '$code', '$time', '0');");
	}
	$msg = "您正在注册，验证码为：".$code."，如非本人操作，请忽略。";
	if($type==2 || $type==3 || $type==4) $msg = "您正找回，验证码为：".$code."，如非本人操作，请忽略。";
	if($type==5) $msg = "您的买单已匹配请及时支付。";
	if($type==6) $msg = "你的卖单已完成，请及时确认。";
	$msg = "【psbc】".$msg;
	run_sql("DELETE FROM `v9_a_sms`  where  fone='$fone' and stat = 0 and addtime < $time1 ");
	run_sql("DELETE FROM `v9_a_sms`  where  stat =1");
	//发送验证码
	send_moble($fone,$msg);
	return $code;
}
...
function send_moble($moble, $content)
{
	...
	$user = '4A5B917B96194230AA532AFE4711E844'; //短信平台帐号
	$pass = '9fd007580793413eb5be762980ef1417'; //短信平台密码
	$time=$_SESSION['time'];
